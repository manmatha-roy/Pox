...
def add_ip_addr (ip_addr, mac):
  if ip_addr in blocked: return
  if ip_addr in unblocked: return
  unblocked.add(ip_addr)
  core.tk.do(unblocked_list.insert, None, END, str(ip_addr) + ' # ' + str(mac))

def packet_handler (event):
  # Note the two MACs
  add_host(event.parsed.next.srcip, event.parsed.src)
  add_host(event.parsed.next.dstip, event.parsed.dst)
