...
def add_ip_addr (ip_addr):
  if ip_addr in blocked: return
  if ip_addr in unblocked: return
  unblocked.add(ip_addr)
  core.tk.do(unblocked_list.insert, None, END, str(ip_addr))

def packet_handler (event):
  # Note the two MACs
  add_ip_addr(event.parsed.next.srcip)
  add_ip_addr(event.parsed.next.dstip)

  # Check for blocked MACs
  if event.parsed.srcip in blocked:
    return EventHalt
  if event.parsed.dstip in blocked:
    return EventHalt
...
def move_entry (from_list, from_set, to_list, to_set):
  ...
  from_list.delete(i)
  to_list.insert(END, ip)
  ip_addr = IpAddr(ip)
  to_set.add(ip_addr)
  from_set.remove(ip_addr)
