...
def add_mac (mac, dpid, port):
  if mac.is_multicast: return
  if mac.is_bridge_filtered: return
  if mac in blocked: return
  if mac in unblocked: return
  unblocked.add(mac)
  core.tk.do(unblocked_list.insert, None, END, str(mac)  + '@' + dpidToStr(dpid) + ':' + str(port))

def packet_handler (event):
  # Note the two MACs
  add_mac(event.parsed.src, event.dpid, event.port)
